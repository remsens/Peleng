...
u::logic HDF5FormatLib::WriteToFile(QString filePath, QString dataSetPath, u::cptr data, u::int64 lines, u::int64 columns)
{
    try
    {
        ...
        dataset_id = H5Dcreate2(groups_idList.at(groups_idList.size()-1), listPathes.at(listPathes.size()-1).toStdString().c_str(), H5T_NATIVE_INT, dataspace_id,
...
u::uint32 HDF5FormatLib::GetDataSetSize(QString filePath, QString dataSetPath)
{
    try
    {
        QStringList listPathes = ParseFile(dataSetPath);
...
u::logic HDF5FormatLib::ReadFromFile(QString filePath, QString dataSetPath, u::ptr data)
{
    try
    {
        QStringList listPathes = ParseFile(dataSetPath);
...
void HDF5FormatLib::GetHDF5StructFile(QString filePathJson1, QString filePathHdf)
{
    hid_t           file;
    herr_t          status;
    H5O_info_t      infobuf;
